...
with pm.Model() as gompertz_model:
    priors = {}
    for param in ['r', 'K']:            # only the parameters you still want to estimate
        sigma = prior_sigmas.get(param)
        initial_condition = initial_prior_conditions.get(param)
        priors[param] = pm.LogNormal(param, mu=np.log(initial_condition), sigma=sigma)

    sigma = pm.HalfNormal('sigma', sigma=prior_sigmas.get('sigma'))

    # Y0 is now a fixed constant, not a random variable
    priors['Y0'] = FIXED_Y0
...
plt.plot(t_plot, ode_solution(t_plot, true_parameter_values),
         color='red', linestyle='--', linewidth=2, zorder=4, label='Curva estimada')

plt.title(f"Estimación bayesiana MCMC de ecuación de {ODE.capitalize()} ", fontsize=14)
